package co.aspirasoft.sams.dao
...
object AttendanceDao {
    ...
    fun add(schoolId: String, attendance: Attendance, listener: OnCompleteListener<Void?>) {
        MyApplication.refToAttendance(schoolId, attendance.classId)
    ...
    fun getByDate(schoolId: String, classId: String, date: String, listener: OnSuccessListener<Attendance?>) {
        MyApplication.refToAttendance(schoolId, classId)
                .child(date)
    ...
    fun getByStudent(schoolId: String, student: Student, listener: OnSuccessListener<List<AttendanceRecord>?>) {
        MyApplication.refToAttendance(schoolId, student.classId)
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
